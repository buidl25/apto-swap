...
export class AptosService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    this.eventEmitter.on('aptos.preimage.discovered', (payload) => {
      this.logger.log(`AptosService received preimage discovery event: ${JSON.stringify(payload)}`);
  ...
  private async setupAptosConnection() {
    try {
      const aptosRpcUrl = this.configService.get<string>("APTOS_RPC_URL") || "https://fullnode.devnet.aptoslabs.com/v1";
      const privateKey = this.configService.get<string>("RELAYER_PRIVATE_KEY_APTOS");
      this.moduleAddress = this.configService.get<string>("APTOS_MODULE_ADDRESS") || "0x6982a96aa68e520bc6f540295e9547689f07eabd89f4ba6aa2a2b45ffc8fa128";
      
      if (!privateKey) {
        throw new Error("RELAYER_PRIVATE_KEY_APTOS must be defined in the environment");
      }

      this.aptosClient = new AptosClient(aptosRpcUrl);
      this.relayerAccount = new AptosAccount(HexString.ensure(privateKey).toUint8Array());
      
      this.logger.log(`Aptos connection established to ${aptosRpcUrl}`);
      this.logger.log(`Relayer address: ${this.relayerAccount.address().hex()}`);
  ...
  async createAptosHtlc(params: {
    ...
    try {
      ...
      const payload = {
        ...
        type_arguments: [`${this.moduleAddress}::test_aptos_token::TestAptosToken`],
        arguments: [
          recipient,
          amount,
          `0x${hashlockHex}`,
  ...
  async withdrawAptosHtlc(params: {
    ...
    try {
      ...
      this.logger.log(`Withdrawing from HTLC with payload: ${JSON.stringify(payload)}`);
  ...
  async checkForWithdrawnEvents(): Promise<{ checked: boolean }> {
    try {
      ...
      this.logger.error(`Failed to check for withdrawn events: ${error.message}`);
