/**
 * Interact with EscrowDst Contract - Withdraw Operation
 * 
 * This script provides functionality to withdraw from the EscrowDst contract
...
async function main() {
  console.log("\n=== Withdrawing from EscrowDst Contract ===\n");
  ...
  await withdrawEscrow(moduleAddress);
}
 ...
 */
async function withdrawEscrow(moduleAddress) {
  const orderHash = process.env.ORDER_HASH;
  if (!orderHash) {
    console.error("ORDER_HASH is required for withdraw. Please set it in .env file.");
    process.exit(1);
  }

  const preimage = process.env.PREIMAGE;
  if (!preimage) {
    console.error("PREIMAGE is required for withdraw. Please set it in .env file.");
    process.exit(1);
  }

  // Вычисляем hashlock из preimage
  const hashlock = '0x' + crypto.createHash('sha3-256').update(Buffer.from(preimage, 'hex')).digest('hex');
  ...
  console.log(`Итоговый ID контракта для withdraw: ${escrowId}`);

  // Выполняем withdraw
  console.log("\nВыполнение withdraw из эскроу...");
  const withdrawCommand = `aptos move run \
    --function-id ${moduleAddress}::escrow_dst::withdraw \
    --type-args 0x1::aptos_coin::AptosCoin \
    --args hex:${escrowId.startsWith('0x') ? escrowId.substring(2) : escrowId} string:${preimage} \
    --profile ${recipientProfile} \
    --assume-yes`;

  console.log(`\nExecuting: ${withdrawCommand}`);
  try {
    const result = execSync(withdrawCommand, { encoding: 'utf8' });
    console.log("Результат:");
    console.log(result);
    console.log("\n✅ Успешно выполнен withdraw из эскроу!");
  } catch (error) {
    console.error("❌ Ошибка при выполнении withdraw из эскроу:");
